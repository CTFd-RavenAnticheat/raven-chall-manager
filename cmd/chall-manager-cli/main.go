				...
				Commands: []*cli.Command{
					{
						...
						Action: func(ctx context.Context, cmd *cli.Command) error {
							...
							resp, err := cliIst.ListCaptureSessions(ctx, &instance.ListCaptureSessionsRequest{
								ChallengeId: cmd.String("challenge_id"),
								SourceId:    cmd.String("source_id"),
							})
							if err != nil {
							...
							fmt.Printf("Capture sessions for <%s,%s>:\n", cmd.String("challenge_id"), cmd.String("source_id"))
							for _, s := range resp.Sessions {
								fmt.Printf("- %s (container: %s, packets: %d, size: %d bytes)\n",
									s.SessionId, s.Container, s.PacketCount, s.FileSize)
						...
						Action: func(ctx context.Context, cmd *cli.Command) error {
							...
							stream, err := cliIst.DownloadCaptures(ctx, &instance.DownloadCapturesRequest{
								ChallengeId: cmd.String("challenge_id"),
								SourceId:    cmd.String("source_id"),
							})
							if err != nil {
							...
							fmt.Printf("[+] Captures downloaded to %s\n", cmd.String("output"))
							return nil
						},
					},
				},
