...
func (mg *Manager) Load(
	...
	mg.Logger.Info("OCI artifact downloaded", zap.Duration("duration", time.Since(dstart)))
	...
	mg.Logger.Info("OCI artifact validated", zap.Duration("duration", time.Since(vstart)))
...
func (mg *Manager) compile(dir string) error {
	mg.Logger.Info("compiling scenario")
	start := time.Now()
	...
	mg.Logger.Info("scenario compiled successfully", zap.Duration("duration", time.Since(start)))
	return nil
}
