...
const (
	...
	InstanceManager_DownloadCaptures_FullMethodName    = "/api.v1.instance.InstanceManager/DownloadCaptures"
	InstanceManager_ListCaptureSessions_FullMethodName = "/api.v1.instance.InstanceManager/ListCaptureSessions"
)
...
type InstanceManagerClient interface {
	...
	// Download packet capture sessions for an instance as a tarball.
	// Returns all capture sessions (up to 3 per container) for the specified instance.
	DownloadCaptures(ctx context.Context, in *DownloadCapturesRequest, opts ...grpc.CallOption) (grpc.ServerStreamingClient[CaptureChunk], error)
	// List capture sessions for an instance.
...
func (c *instanceManagerClient) DownloadCaptures(ctx context.Context, in *DownloadCapturesRequest, opts ...grpc.CallOption) (grpc.ServerStreamingClient[CaptureChunk], error) {
	cOpts := append([]grpc.CallOption{grpc.StaticMethod()}, opts...)
	stream, err := c.cc.NewStream(ctx, &InstanceManager_ServiceDesc.Streams[1], InstanceManager_DownloadCaptures_FullMethodName, cOpts...)
	if err != nil {
		return nil, err
	}
	x := &grpc.GenericClientStream[DownloadCapturesRequest, CaptureChunk]{ClientStream: stream}
	if err := x.ClientStream.SendMsg(in); err != nil {
		return nil, err
	}
	if err := x.ClientStream.CloseSend(); err != nil {
...
// This type alias is provided for backwards compatibility with existing code that references the prior non-generic stream type by name.
type InstanceManager_DownloadCapturesClient = grpc.ServerStreamingClient[CaptureChunk]

func (c *instanceManagerClient) ListCaptureSessions(ctx context.Context, in *ListCaptureSessionsRequest, opts ...grpc.CallOption) (*ListCaptureSessionsResponse, error) {
	cOpts := append([]grpc.CallOption{grpc.StaticMethod()}, opts...)
	out := new(ListCaptureSessionsResponse)
	err := c.cc.Invoke(ctx, InstanceManager_ListCaptureSessions_FullMethodName, in, out, cOpts...)
	if err != nil {
		return nil, err
	}
	return out, nil
...
type InstanceManagerServer interface {
	...
	// Download packet capture sessions for an instance as a tarball.
	// Returns all capture sessions (up to 3 per container) for the specified instance.
	DownloadCaptures(*DownloadCapturesRequest, grpc.ServerStreamingServer[CaptureChunk]) error
	// List capture sessions for an instance.
...
func (UnimplementedInstanceManagerServer) DownloadCaptures(*DownloadCapturesRequest, grpc.ServerStreamingServer[CaptureChunk]) error {
	return status.Error(codes.Unimplemented, "method DownloadCaptures not implemented")
}
func (UnimplementedInstanceManagerServer) ListCaptureSessions(context.Context, *ListCaptureSessionsRequest) (*ListCaptureSessionsResponse, error) {
...
func _InstanceManager_DownloadCaptures_Handler(srv interface{}, stream grpc.ServerStream) error {
	m := new(DownloadCapturesRequest)
	if err := stream.RecvMsg(m); err != nil {
		return err
	}
	return srv.(InstanceManagerServer).DownloadCaptures(m, &grpc.GenericServerStream[DownloadCapturesRequest, CaptureChunk]{ServerStream: stream})
}

// This type alias is provided for backwards compatibility with existing code that references the prior non-generic stream type by name.
type InstanceManager_DownloadCapturesServer = grpc.ServerStreamingServer[CaptureChunk]

func _InstanceManager_ListCaptureSessions_Handler(srv interface{}, ctx context.Context, dec func(interface{}) error, interceptor grpc.UnaryServerInterceptor) (interface{}, error) {
	in := new(ListCaptureSessionsRequest)
	if err := dec(in); err != nil {
		return nil, err
	}
	if interceptor == nil {
		return srv.(InstanceManagerServer).ListCaptureSessions(ctx, in)
	}
	info := &grpc.UnaryServerInfo{
		Server:     srv,
		FullMethod: InstanceManager_ListCaptureSessions_FullMethodName,
	}
	handler := func(ctx context.Context, req interface{}) (interface{}, error) {
		return srv.(InstanceManagerServer).ListCaptureSessions(ctx, req.(*ListCaptureSessionsRequest))
	}
	return interceptor(ctx, in, info, handler)
}
...
var InstanceManager_ServiceDesc = grpc.ServiceDesc{
	...
	Methods: []grpc.MethodDesc{
		...
		},
		{
			...
			Handler:    _InstanceManager_ListCaptureSessions_Handler,
		},
	},
	Streams: []grpc.StreamDesc{
		...
		{
			StreamName:    "DownloadCaptures",
			Handler:       _InstanceManager_DownloadCaptures_Handler,
			ServerStreams: true,
		},
	},
