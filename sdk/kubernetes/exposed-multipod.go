...
//go:embed assets/capture-daemon.sh
var captureDaemonScript string
...
func (emp *exposedMultipod) provision(ctx *pulumi.Context, args ExposedMultipodArgsOutput, opts ...pulumi.ResourceOption) (gerr error) {
	...
	for i := 0; i < lenP(order); i++ {
		...
		labels := pulumi.StringMap{
			...
			"chall-manager.ctfer.io/challenge": args.ChallengeID(),
		...
		// Deterministic names for packet capture resources
		pcapScriptName := pulumi.All(args.Identity(), args.Label(), name).ApplyT(func(all []any) string {
			id := all[0].(string)
			name := all[2].(string)
			if lbl, ok := all[1].(string); ok && lbl != "" {
				return fmt.Sprintf("emp-pcap-script-%s-%s-%s", lbl, id, name)
			}
			return fmt.Sprintf("emp-pcap-script-%s-%s", id, name)
		}).(pulumi.StringOutput)

		// => ConfigMap for packet capture daemon script (if packet capture enabled)
		container.PacketCapture().ApplyT(func(pc *bool) error {
			if pc != nil && *pc {
				_, err := corev1.NewConfigMap(ctx, fmt.Sprintf("emp-pcap-script-%s", rawName), &corev1.ConfigMapArgs{
					Immutable: pulumi.BoolPtr(true),
					Metadata: metav1.ObjectMetaArgs{
						Name:   pcapScriptName,
						Labels: labels,
			...
			Spec: appsv1.DeploymentSpecArgs{
				...
				Template: &corev1.PodTemplateSpecArgs{
					...
					Spec: &corev1.PodSpecArgs{
						...
						ShareProcessNamespace:        container.PacketCapture(),
						ImagePullSecrets: args.ImagePullSecrets().ApplyT(func(secrets []string) []corev1.LocalObjectReference {
							if len(secrets) == 0 {
								return nil
							}
						...
						Containers: pulumi.All(args.Identity(), container.Ports(), container.PacketCapture(), name, args.Label()).ApplyT(func(all []any) corev1.ContainerArray {
							...
							// Add packet capture sidecar if enabled
							if packetCapture != nil && *packetCapture {
								// Build port list for the daemon
								portList := ""
								for i, pb := range pbs {
									if i > 0 {
										portList += ","
									...
									portList += fmt.Sprintf("%d:%s", pb.Port, protocol)
								}

								captureDir := "/captures"
								labelStr := ""
								if lbl, ok := label.(string); ok && lbl != "" {
									labelStr = lbl
								}
								...
								containers = append(containers, corev1.ContainerArgs{
									...
									Env: corev1.EnvVarArray{
										corev1.EnvVarArgs{
											...
											Value: pulumi.String(containerName),
										},
										corev1.EnvVarArgs{
										...
										corev1.EnvVarArgs{
											...
											Value: pulumi.String(labelStr),
										},
										corev1.EnvVarArgs{
											Name:  pulumi.String("PORTS"),
											Value: pulumi.String(portList),
										},
										corev1.EnvVarArgs{
											Name:  pulumi.String("CAPTURE_DIR"),
											Value: pulumi.String(captureDir),
										},
									},
									SecurityContext: corev1.SecurityContextArgs{
										Privileged: pulumi.BoolPtr(true),
										RunAsUser:  pulumi.IntPtr(0),
										Capabilities: corev1.CapabilitiesArgs{
											Add: pulumi.ToStringArray([]string{"NET_RAW", "NET_ADMIN"}),
										},
									},
									VolumeMounts: pulumi.All(args.ChallengeID(), args.Identity()).ApplyT(func(all []any) []corev1.VolumeMount {
										challID := all[0].(string)
										id := all[1].(string)

										return []corev1.VolumeMount{
											{
												Name:      "packet-captures",
												MountPath: captureDir,
												SubPath:   ptr(fmt.Sprintf("captures/%s/%s/%s", challID, id, containerName)),
											},
											{
												Name:      "capture-script",
												MountPath: "/scripts",
						...
						Volumes: pulumi.All(vs, container.PacketCapture(), pcapScriptName, args.PacketCapturePVC()).ApplyT(func(all []any) []corev1.Volume {
							volumes := all[0].([]corev1.Volume)
							packetCapture := all[1].(*bool)
							scriptName := all[2].(string)
							sharedPVC := all[3].(*string)

							// Add persistent volume for packet captures if enabled
							if packetCapture != nil && *packetCapture {
								targetPVC := "pcap-core"
								if sharedPVC != nil {
									targetPVC = *sharedPVC
...
func (o ExposedMultipodArgsOutput) ChallengeID() pulumi.StringOutput {
	return o.ApplyT(func(args ExposedMultipodArgsRaw) string {
		return args.ChallengeID
	}).(pulumi.StringOutput)
}

func (o ExposedMultipodArgsOutput) PacketCapturePVC() pulumi.StringPtrOutput {
	return o.ApplyT(func(args ExposedMultipodArgsRaw) *string {
		return args.PacketCapturePVC
	}).(pulumi.StringPtrOutput)
}

func (o ExposedMultipodArgsOutput) ImagePullSecrets() pulumi.StringArrayOutput {
	return o.ApplyT(func(args ExposedMultipodArgsRaw) []string {
